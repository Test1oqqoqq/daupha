...
module.exports = class {
    ...
    static async onRun({ api, event, args }) {
        ...
        if (sub === "douqi") {
            ...
            return api.sendMessage(`Các cấp bậc Đấu khí:\n${DOUQI.map((d, i) => `${i + 1}. ${d}`).join("\n")}\n\nChọn: {pn}dauphaaa douqi chon [id] (cần đủ EXP, lên cấp nhận xu, bonus nếu có sư phụ)`, event.threadID, event.messageID);
        }
        ...
        if (sub === "giatoc") {
            ...
            return api.sendMessage(`Các gia tộc lớn:\n${GIATOC.map((g, i) => `${i + 1}. ${g}`).join("\n")}\n\nChọn: {pn}dauphaaa giatoc chon [id] (mất 200 xu, mỗi ngày 1 lần)`, event.threadID, event.messageID);
        }
        ...
        if (sub === "luyenduoc") {
            ...
            return api.sendMessage(`Các loại dược phẩm nổi bật:\n${LUYENDUOC.map((i, idx) => `${idx + 1}. ${i.name} - ${i.desc}`).join("\n")}\n\nLuyện: {pn}dauphaaa luyenduoc chon [id] (mỗi lần luyện tốn 100 xu, có thể thất bại)`, event.threadID, event.messageID);
        }
        ...
        if (sub === "tinh") {
            ...
            return api.sendMessage(`Đấu phá vượt cảnh giới:\n${TINH.map((t, i) => `${i + 1}. ${t}`).join("\n")}\n\nChọn: {pn}dauphaaa tinh chon [id] (cần đủ EXP, đạt tinh nhận xu)`, event.threadID, event.messageID);
        }
        ...
        return api.sendMessage("Lệnh không hợp lệ. Dùng {pn}dauphaaa menu để xem hướng dẫn.", event.threadID, event.messageID);
    }
